    ...
    <div>
        ...
        <div v-else-if="!role" class="panel">
            <div class="text-center py-20">
                ...
                <h3 class="text-lg font-semibold text-gray-600 dark:text-gray-400 mb-2">Role Not Found</h3>
        ...
        <div v-else class="panel">
            ...
            <div v-if="isProtected" class="flex items-center p-3.5 rounded text-warning bg-warning-light dark:bg-warning-dark-light mb-5" role="alert">
                <icon-info-triangle class="w-5 h-5 mr-3 shrink-0" />
                <span>{{ $t('roles.protected_role_warning') }}</span>
            </div>
            ...
            <form @submit.prevent="handleSubmit" class="space-y-5">
                ...
                <div>
                    ...
                    <div v-if="isProtected" class="flex items-center p-3 rounded text-warning bg-warning-light dark:bg-warning-dark-light mb-3">
                        <icon-info-triangle class="w-4 h-4 mr-2 shrink-0" />
                        <span class="text-sm">{{ $t('roles.cannot_modify_protected_permissions') }}</span>
                    </div>
...
onMounted(async () => {
    ...
    try {
        ...
        if (roleStore.currentRole) {
            ...
            form.permissions = role.value.permissions?.map(p => p.name) || [];
            isProtected.value = roleStore.isProtectedRole(role.value.name);
        } else {
